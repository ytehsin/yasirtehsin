...
import { toDocx, conversionNotice, extensionOf } from "../import/convert";
...
async function handleImport(req: any, res: any, url: URL) {
  ...
  // Anything that isn't already a .docx gets converted first. Refusals come
  // back as a sentence the person can act on — "unsupported file" tells them
  // nothing and they will try the same file again.
  let docx;
  try {
    docx = await toDocx(buf, filename);
  } catch (err: any) {
    return json(res, 400, { error: err.message });
  }

  const id = randomUUID();
  const result = await importDocx(docx.buffer, id);
  ...
  return json(res, 200, {
    ...
    notices: [
      ...(docx.converted
        ? [{ severity: "info", message: conversionNotice(docx.from) }]
        : []),
      ...result.notices,
    ],
