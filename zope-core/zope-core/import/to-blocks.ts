  ...
  return groups
    ...
    .map((g, i) => {
      ...
      return {
        ...
        bookId,
        order: i,
        // A group with no boundary is content sitting before the first
        // detected chapter. Converted files often start with a stray empty
        // paragraph, and calling that "Untitled" makes the confirmation screen
        // look broken. Naming it for what it is keeps the words (nothing is
        // dropped) and tells the person what they are looking at.
        kind: b?.kind ?? (i === 0 ? "preface" : "chapter"),
        title: b?.title ?? (i === 0 ? "Front matter" : `Section ${i + 1}`),
        number: b?.number,
        lang,
